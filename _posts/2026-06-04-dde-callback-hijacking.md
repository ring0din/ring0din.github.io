...
tags: [dde, ddeml, process-injection, win32k, windows-internals]
toc: true
---
...
Existing offensive research on DDE splits in two. The Office-document angle, `WM_DDE_EXECUTE` command execution (MITRE ATT&CK [T1559.002](https://attack.mitre.org/techniques/T1559/002/)), is unrelated to what follows. The directly relevant work is odzhan/modexp's 2019 [**Breaking BaDDEr**](https://modexp.wordpress.com/2019/08/09/windows-process-injection-breaking-badder/), which already targets exactly the structure this post does: it overwrites the DDEML callback (`pfnCallback`) inside the undocumented `CL_INSTANCE_INFO` / `WDML_INSTANCE` instance struct to get code execution in `explorer.exe`. So the *target* is not new, and I am not claiming it is. What this post adds is incremental. Instead of odzhan's `DDEMLMom` mother window, I reach the same field by walking the per-conversation `DDEMLUnicodeServer` / `DDEMLAnsiServer` windows (`EWM[0] → tagCONV_INFO+0x008 → tagCL_INSTANCE_INFO+0x040`). On top of that: an `XTYP_EXECUTE` trigger, dynamic offset resolution, and a closer look at the `GetWindowLongPtr` read as a win32k-serviced pointer disclosure. Treat this as a refinement of a known 2019 technique, not a new primitive and not a new target.
...
DDE is not just a document-macro vector. One internal DDEML structure, the `WDML_INSTANCE` callback pointer reachable across processes through Extra Window Memory, is a process injection surface that thread-injection-focused detection does not cover. This is the target odzhan's 2019 [Breaking BaDDEr](https://modexp.wordpress.com/2019/08/09/windows-process-injection-breaking-badder/) first hit, and it sits in the same family as Extra Window Memory Injection (`T1055.011`): the same pivot through EWM, but you overwrite a DDEML heap callback instead of the window's own extra bytes, and you trigger it with a client DDE transaction instead of a window-proc message. One honest caveat: classic EWM injection avoids `WriteProcessMemory` by writing into the window's own extra bytes, whereas this technique uses a cross-process write, so it is not the stealthier of the two on the write. The write phase stays detectable. What is incremental here over Breaking BaDDEr is the access path and the framing of the disclosure, not the target.
...
- **Prior art.** Overwriting the DDEML `pfnCallback` in `CL_INSTANCE_INFO` / `WDML_INSTANCE` for code execution was first published by odzhan/modexp in [*Windows Process Injection: Breaking BaDDEr*](https://modexp.wordpress.com/2019/08/09/windows-process-injection-breaking-badder/) (2019; PoC at [odzhan/injection](https://web.archive.org/web/20221211161227/https://github.com/odzhan/injection/tree/master/dde), catalogued as Unprotect U1201). That work owns the target and the core primitives: the `GetWindowLongPtr` Extra-Window-Memory read to locate the struct, the `WriteProcessMemory` overwrite of the callback, the DDE trigger, and the pointer restore. The broader family is `T1055.011` (Extra Window Memory Injection), with PowerLoader/EWMI and Hexacorn's PROPagate as adjacent function-pointer-in-window-state techniques.
- **What this post contributes** over Breaking BaDDEr is narrow, and I am labeling it as such: a different access path to the same field (per-conversation `DDEMLUnicodeServer` / `DDEMLAnsiServer` windows → `tagCONV_INFO+0x008` → `tagCL_INSTANCE_INFO+0x040`, versus odzhan's `DDEMLMom` window), the `XTYP_EXECUTE` trigger, dynamic module-aware offset resolution instead of hardcoded offsets, and the analysis of the `GetWindowLongPtr` read as a no-`OpenProcess`, win32k-serviced disclosure. Whether the specific offset chain or any non-`explorer.exe` DDEML host is genuinely first-published here, I am not asserting without a dedicated prior-art pass.
